      ...
      <div style={styles} >
        ...
        <div className='row mt-5 '>
          <div className='col-5 offset-1 mt-5'  >
            <h1 style={{ fontSize: "30px", fontWeight: "bold" }} className='mb-4 text-white'>Billing Details</h1>
            <div class="mb-3">
              <label for="exampleFormControlInput1" class="form-label text-white">Contact </label>
              <input type="email" class="form-control" id="exampleFormControlInput1" placeholder="mobile number" onChange={(e) => this.setState({ contact: e.target.value })} required />
            </div>
            <div class="mb-3">
              <label for="exampleFormControlTextarea1" class="form-label text-white">Address</label>
              <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" onChange={(e) => this.setState({ address: e.target.value })} required></textarea>
            </div>
          </div>
          <div className='col-6 mt-5'>
            <center>
              <h2 style={{ fontSize: "30px", fontWeight: "bold" }} className='text-white mt-3'>Your order</h2>
              <div className="col-5 mt-4" style={{ backgroundColor: "#fff" }}>
                <form>
                  <div className="mb-3 mt-3">
                    <span style={{ fontSize: "20px", fontWeight: "bold" }} className="form-label">Oder Number: <span>0999324</span> </span>
                  </div>
                  <div className="mb-3">
                    <span style={{ fontSize: "20px", fontWeight: "bold" }} className="form-label">Customer: <span>{JSON.parse(localStorage.getItem('user')).name}</span></span>
                  </div>
                  <div className="mb-3">
                    <span style={{ fontSize: "20px", fontWeight: "bold" }} className="form-label">Total: Rs.<span>{localStorage.getItem('cartTotal')}</span>.00</span>
                  </div>
                </form>
              </div>
            </center>
          ...
          <div className='col-4 offset-2' style={{ height: "20px" }}>
            <button type="button" className='btn btn-dark col-12 mt-5' style={{ height: "50px" }} onClick={async () => { await this.addOrder(); }} >Check Out</button>
          </div>
          <div className='col-2 offset-1' style={{ height: "200px", marginLeft: "265px" }}>
            <StripeCheckout
